...
class KerasTrainClassifierTestCase(unittest.TestCase):
        ...
        with tempfile.TemporaryDirectory() as temp_dir_name:
            with components_local_output_dir_context(temp_dir_name):
                train_task = train_op(
                    ...
                    time_to_columns=str(["week"]),
                    out_timestamp_path="file://"+os.path.join(testdata_root,"time.parquet")
                )

            full_command = train_task.command + train_task.arguments
            full_command[0] = 'python'
            full_command[1] = os.path.join(component_root, 'src', 'time_preprocess.py')

            process = subprocess.run(full_command)

            (output_model_uri_file,) = (train_task.file_outputs['out_timestamp_path'],)
    ...
    def test_2(self):
        ...
        testdata_root = os.path.abspath(os.path.join(tests_root, 'testdata/time.parquet'))
