        ...
        with tempfile.TemporaryDirectory() as temp_dir_name:
            with components_local_output_dir_context(temp_dir_name):
                train_task = train_op(
                    ...
                    label_name="rating",
                    save_model_path="/tmp/automl_spark/8e091568-2a9f-4ce5-ac75-c4ffa61a7a20/pipeline-sm6jc-3713971364/model.out"
                )

            full_command = train_task.command + train_task.arguments
            full_command[0] = 'python'
            full_command[1] = os.path.join(component_root, 'src', 'verification.py')
