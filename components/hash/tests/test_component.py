...
class KerasTrainClassifierTestCase(unittest.TestCase):
        ...
        with tempfile.TemporaryDirectory() as temp_dir_name:
            with components_local_output_dir_context(temp_dir_name):
                train_task = train_op(
                    ...
                    out_hash_path="file://"+os.path.join(testdata_root,"hash.parquet"),
                    entilty_list=str(["userId","movieId","timestamp"])
                )

            full_command = train_task.command + train_task.arguments
            full_command[0] = 'python'
            full_command[1] = os.path.join(component_root, 'src', 'hash.py')

            process = subprocess.run(full_command)

            (output_model_uri_file,) = (train_task.file_outputs['out_hash_path'],)
    ...
    def test_2(self):
        ...
        testdata_root = os.path.abspath(os.path.join(tests_root, 'testdata/hash.parquet'))
